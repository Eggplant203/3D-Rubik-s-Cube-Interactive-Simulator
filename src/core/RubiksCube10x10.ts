...
export class RubiksCube10x10 extends RubiksCube {
  ...
  public stopAnimation(): void {
    ...
    // Handle rotating group
    if ((this as any).currentRotationGroup) {
      // Move cubelets back to cube group
      (this as any).currentRotationGroup.children.slice().forEach((child: THREE.Object3D) => {
        this.getCubeGroup().add(child);
      });
      (this as any).currentRotationGroup = null;
    }
